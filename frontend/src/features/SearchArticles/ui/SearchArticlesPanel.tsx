...
const SearchArticlesPanel = () => {
  const { draftFilters, sortType, applyDraftFilters } = useArticle();

  const [isSubmitButtonAcitve, setIsSubmitButtonAcitve] =
    useState<boolean>(false);

  useEffect(() => {
    const { title, authors, extractedText } = draftFilters;
  ...
  return (
    <div className="flex justify-center">
      <div className="w-full max-h-auto box-border p-10 bg-accent-dim rounded-2xl flex flex-col items-start gap-20">
        ...
        <button
          className={`box-border pl-6 pr-6 pt-4 pb-4 rounded-3xl border-2 
                        ${
                          isSubmitButtonAcitve
                            ? "bg-accent-third border-accent-secondary text-text cursor-pointer"
                            : "bg-accent-first border-accent-first text-text-grey-dim"
                        } `}
          onClick={applyFilters}
        >
          <span className="font-base text-4xl">Искать</span>
        </button>
      </div>
    </div>
  );
