...
const ArticlePreview: React.FC<ArticlePreviewProps> = ({ article }) => {
    ...
    return (
        <Link to={`${article.slug}`}>
            <div className="max-h-250 relative rounded-3xl border-2 border-accent-third overflow-hidden group ">
                        ...
                        <div className="flex flex-col gap-7 opacity-0 translate-y-20 transition-all duration-500 group-hover:translate-y-0 group-hover:opacity-100">
                            <div>
                                <span className="text-2xl text-text-grey font-base-light font-bold">
                                    {formatted} | {article.authors.length > 0 ? article.authors.map(author => author.name).join(", ") : 'Автор не указан'}
                                </span>
                            </div>
                            <div className="flex gap-5 items-center">
                                <span className="font-base-light font-bold text-accent-secondary text-2xl">
                                    {`Подробнее ------>`}
                                </span>
                            </div>
                        </div>
