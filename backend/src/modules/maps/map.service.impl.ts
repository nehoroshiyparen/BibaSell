...
export class MapServiceImpl implements IMapService {
    ...
    async getList(
        ...
        try {
            ...
            if (filters.description) where.title = { [Op.iLike]: `%${filters.description}%` }
            if (filters.year) where.title = { [Op.iLike]: `%${filters.year}%` }
            ...
            RethrowApiError(`Service error: Method - getMapsFiltered`, e)
        }
    }
    ...
    async update(
        ...
        try {
            ...
            const updateData: Partial<TypeofMapUpdateSchema & { slug: string }> = cleanup({
                title: options.title,
            ...
            const updatedMap = await this.sequelize.update(id, updateData, transaction)
    ...
    async bulkDelete(ids: number[]): Promise<OperationResult> {
        ...
        try {
            ...
            for (const id of missingIds) {
                errorStack[id] = { message: `Map with id ${id} not found`, code: 'MAP_NOT_FOUND' }
            }
            ...
            for (const map of maps) {
                if (map.key) {
                    try {
                        ...
                        errorStack[map.id] = { message: 'S3 delete failed', code: 'S3_ERROR' }
                    }
                }
            }
