...
export class MapControllerImpl {
  constructor(@inject(TYPES.MapService) private mapService: IMapService) {}
  ...
  async getMaps(req: Request, res: Response) {
    try {
      ...
      const filters: Partial<TypeofMapFiltersSchema> = {
        ...
        year: Number(req.query.year?.toString()),
      };
  ...
  async createMap(req: Request, res: Response) {
    try {
      ...
      if (!hasFiles) {
        throw new ApiError(
          status.BAD_REQUEST,
          "Map cant be created without map file. Map file is not attached",
        );
      }
  ...
  async updateMap(req: Request, res: Response) {
    try {
      ...
      const hasOptions =
        validatedOptions && Object.keys(validatedOptions).length > 0;

      if (!hasOptions && !hasFiles) {
        throw new ApiError(
          status.BAD_REQUEST,
          "At least one option param has to be specified or file needs to be attached",
        );
      }
      const map = await this.mapService.update(
        id,
        validatedOptions,
        fileConfig,
      );
        ...
        cases: [
          ...
          },
          {
            condition: () =>
              ...
              bulkDeleteResult.errors.length < ids.length,
            status: 206,
