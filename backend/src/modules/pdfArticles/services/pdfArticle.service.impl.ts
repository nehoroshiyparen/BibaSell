...
import { cleanText } from "../utils/files/cleanText.js";
...
export class PdfArticleServiceImpl implements IPdfArticleService {
  constructor(
    @inject(TYPES.PdfArticleSequelizeRepo)
    private sequelize: PdfArticleSequelizeRepo,
  ...
  async getById(id: number): Promise<TypeofPdfArticleFullSchema> {
    try {
      ...
      if (!article)
        throw ApiError.NotFound(`Article with id: ${id} was not found`);
  ...
  async getBySlug(slug: string): Promise<TypeofPdfArticleFullSchema> {
    try {
      ...
      if (!article)
        throw ApiError.NotFound(`Article with slug: ${slug} was not found`);
      return await this.mapper.toFull(article);
    } catch (e) {
  ...
  async getList(
    ...
    try {
      ...
      const extractedText = cleanText(
        filters.extractedText || (filters as any).content,
      );
      const author = cleanText(
        filters.author ||
          (Array.isArray((filters as any).authors)
            ? (filters as any).authors[0]
            : ""),
      );
      ...
      if (Object.keys(elasticQuery).length > 0) {
        const elasticCandidates =
          await this.elastic.searchArticles(elasticQuery);
  ...
  async create(
    ...
    try {
      ...
      const previewData = await readFileFromConfig(
        fileConfig,
        "preview",
        false,
      );
      ...
      if (!pdfData)
        throw ApiError.BadRequest(
          "Article cant be created without file. File is not attached",
        );

      const extractedText = cleanText(await pdfParse(pdfData));

      if (!options.title)
        throw ApiError.BadRequest("Article must have a title");
      ...
      if (previewData)
        await this.s3.upload(previewKey, previewData, {
          prefix: "article_previews/",
        });
  ...
  async update(
    ...
    const article = await this.sequelize.findById(id);
    if (!article)
      throw ApiError.NotFound(`Article with id: ${id} is not found`);

    try {
      const pdfFile = fileConfig.files.pdf as Express.Multer.File | undefined;
      const previewFile = fileConfig.files.preview as
        | Express.Multer.File
        | undefined;
      ...
      const updatedArticle = await this.sequelize.update(
        id,
        updateData,
        transaction,
      );
  ...
  private async updatePreviewFile(
    ...
    await this.s3.upload(previewKey, previewBuffer, {
      prefix: "article_previews/",
    });
  ...
  private async rollbackElasticChanges(oldArticle: PdfArticle) {
    try {
      ...
      throw ApiError.Internal(
        `Failed to rollback changes in Elastic, while updating Pdf Article with id: ${oldArticle.id}`,
        undefined,
        e,
      );
    }
